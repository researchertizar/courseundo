(function () {
    ...
    function cacheDom() {
        ...
        mobileMenuBtn = gid('mobile-menu-btn');
        mobileNavOverlay = gid('mobile-nav-overlay');
        mobileNavClose = gid('mobile-nav-close');
        mobileFilterDone = gid('mobile-filter-done');
        fabFilter = gid('fab-filter');
        filterToggleBtn = gid('filter-toggle-btn');
        sidebar = gid('sidebar');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        /* Verify connection */
        fetch(SUPA_URL + '/rest/v1/courses?limit=1', { headers: { 'apikey': ANON_KEY } })
            .then(function (r) { console.log('[Courseundo] Connection:', r.status); })
            .catch(function (e) { console.error('[Courseundo] Connection failed:', e); });
        ...
        if (saved && sortSel) { S.sort = saved; sortSel.value = saved; }
    ...
    function bindEvents() {
        ...
        for (var i = 0; i < fkeys.length; i++) {
            (function (k) {
                on(filtEls[k], 'change', function () {
                    ...
                    S.page = 1;
                    loadCourses();
                    /* Close mobile sidebar on filter */
        ...
        for (var t = 0; t < tabBtns.length; t++) {
            (function (btn) { on(btn, 'click', function () { switchTab(btn.getAttribute('data-tab')); }); })(tabBtns[t]);
        }
        ...
        on(mobileMenuBtn, 'click', function () {
            var active = mobileNavOverlay.classList.toggle('active');
            mobileMenuBtn.classList.toggle('active', active);
            mobileNavOverlay.setAttribute('aria-hidden', !active);
            mobileMenuBtn.setAttribute('aria-expanded', active);
        ...
        on(filterToggleBtn, 'click', toggleSidebar);
        on(fabFilter, 'click', toggleSidebar);
        on(mobileFilterDone, 'click', function () { if (sidebar) sidebar.classList.remove('open'); });
    }

    function closeMobileNav() {
        if (mobileNavOverlay) mobileNavOverlay.classList.remove('active');
        if (mobileMenuBtn) { mobileMenuBtn.classList.remove('active'); mobileMenuBtn.setAttribute('aria-expanded', 'false'); }
    ...
    function clientFilt(arr) {
        return arr.filter(function (c) { for (var k in S.filt) { if (S.filt[k] && (c[k] || '').toLowerCase() !== S.filt[k].toLowerCase()) return false; } return true; });
    }
    ...
    function renderCourses() {
        ...
        for (var i = 0; i < S.courses.length; i++) {
            ...
            h += '<article class="course-card" style="animation-delay:' + (i * 0.04).toFixed(2) + 's">' +
                ...
                '<h3 class="card-title"><a href="' + escA(c.link) + '" target="_blank" rel="noopener" class="clink" data-id="' + c.id + '" data-t="' + escA(c.title) + '">' + esc(c.title) + '</a></h3>' +
                (c.platform ? '<span class="card-platform ' + platC(c.platform) + '">' + esc(c.platform) + '</span>' : '') +
                '</div>' +
                buildBadges(c) + buildMeta(c) +
                '<div class="card-rating">' +
                '<div class="stars cstars" data-id="' + c.id + '" data-t="' + escA(c.title) + '" role="button" tabindex="0">' + buildStars(c.rating_avg || 0) + '</div>' +
        ...
        for (var li = 0; li < links.length; li++) {
            (function (a) { on(a, 'click', function () { logAct('course_click', { id: a.getAttribute('data-id'), title: a.getAttribute('data-t') }); }); })(links[li]);
        }
    ...
    function renderPages() {
        ...
        for (var j = 0; j < btns.length; j++) {
            (function (b) { on(b, 'click', function () { var p = parseInt(b.getAttribute('data-p'), 10); if (p >= 1 && p <= tp) { S.page = p; loadCourses(); window.scrollTo({ top: 0, behavior: 'smooth' }); } }); })(btns[j]);
        }
    }
    ...
    function updMode() {
        ...
        searchMode.innerHTML = S.stype === 'semantic' ? '<span class="dot"></span> AI semantic search' : '<span class="dot" style="background:var(--text-muted)"></span> Keyword search';
    }

    function clearFilters() {
        var keys = Object.keys(S.filt);
        for (var i = 0; i < keys.length; i++) { S.filt[keys[i]] = ''; if (filtEls[keys[i]]) { filtEls[keys[i]].value = ''; filtEls[keys[i]].classList.remove('has-value'); } }
        S.page = 1;
        loadCourses();
    }
    ...
    function loadSugs() {
        apiGet('suggestions', { status: 'eq.pending', order: 'created_at.desc' }).then(function (r) {
            ...
            for (var i = 0; i < r.data.length; i++) {
                var s = r.data[i];
                h += '<div class="suggestion-card" style="animation-delay:' + (i * 0.04).toFixed(2) + 's"><div class="flex-between"><h4 class="suggestion-title"><a href="' + escA(s.link) + '" target="_blank">' + esc(s.title) + '</a></h4><span class="badge badge-pending">Pending</span></div><div class="suggestion-meta">' + (s.platform ? '<span>' + esc(s.platform) + '</span>' : '') + (s.user_name ? '<span>By ' + esc(s.user_name) + '</span>' : '') + '<span>' + fmtD(s.created_at) + '</span></div>' + (s.notes ? '<p class="suggestion-notes">"' + esc(s.notes) + '"</p>' : '') + '</div>';
            }
            sugList.innerHTML = h;
        }).catch(function () { });
    }
    ...
    function rateClick(e) {
        ...
        apiPost('ratings', { course_id: pRate.id, rating: v }, false)
            .then(function () { toast('Rated ' + v + ' stars!', 'success'); closeRate(); logAct('rating', { id: pRate.id, rating: v }); loadCourses(); })
    ...
    function loadStats() {
        apiGet('courses', { status: 'eq.active', select: 'id', limit: 1 }).then(function (r) {
            ...
            var mc = gid('mobile-course-count');
            if (mc) mc.textContent = r.total || 0;
        }).catch(function () { });
    }
