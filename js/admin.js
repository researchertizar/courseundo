(function () {
    ...
    function cacheDom() {
        ...
        for (var i = 0; i < names.length; i++) {
            fields[names[i]] = gid('course-' + names[i]);
        }
    }
    ...
    function aPost(ep, body) {
        return fetch(REST + '/' + ep, { method: 'POST', headers: authH(), body: JSON.stringify(body) })
            .then(function (r) { return r.json().then(function (d) { if (!r.ok) throw new Error(d.message || 'Error ' + r.status); return d; }); });
    }

    function aPatch(ep, body, params) {
        params = params || {};
        var url = REST + '/' + ep;
        var first = true;
        for (var k in params) { url += (first ? '?' : '&') + encodeURIComponent(k) + '=' + encodeURIComponent(params[k]); first = false; }
        return fetch(url, { method: 'PATCH', headers: authH(), body: JSON.stringify(body) })
            .then(function (r) { return r.json().then(function (d) { if (!r.ok) throw new Error(d.message || 'Error ' + r.status); return d; }); });
    }
    ...
    function renderList() {
        ...
        for (var j = 0; j < items.length; j++) {
            (function (el) { on(el, 'click', function () { editCourse(el.getAttribute('data-id')); }); })(items[j]);
        }
    }
    ...
    function editCourse(id) {
        ...
        for (var j = 0; j < fkeys.length; j++) {
            if (fields[fkeys[j]]) fields[fkeys[j]].value = c[ck(fkeys[j])] || '';
        }
    ...
    function saveCourse(e) {
        ...
        if (extraJson && extraJson.value.trim()) {
            try { extra = JSON.parse(extraJson.value.trim()); } catch (je) { toast('Invalid JSON in Extra Fields.', 'error'); return; }
        }

        var body = { extra_fields: extra };
        var fkeys = Object.keys(fields);
        for (var i = 0; i < fkeys.length; i++) {
            var v = fields[fkeys[i]] ? fields[fkeys[i]].value.trim() : '';
            if (v) body[ck(fkeys[i])] = v;
        }

        var p;
        if (S.editId) {
            p = aPatch('courses', body, { id: 'eq.' + S.editId });
        } else {
            body.status = 'active';
            p = aPost('courses', body);
        }
        ...
        p.then(function (res) {
            ...
            if (cd && cd.id) {
                aFn('generate-embedding', { course_id: cd.id, title: body.title || '', description: '', category: body.category || '' }).catch(function () { });
            }
            resetForm();
            loadCourses();
    ...
    function doAiClassify() {
        ...
        aFn('classify-course', { title: t, description: '' }).then(function (d) {
            ...
            .finally(function () { if (aiClassBtn) { aiClassBtn.disabled = false; aiClassBtn.textContent = 'AI Classify'; } });
    }

    function doArchive() {
        if (!S.editId || !confirm('Archive this course?')) return;
        aPatch('courses', { status: 'archived', embedding: null }, { id: 'eq.' + S.editId })
            .then(function () { toast('Archived.', 'success'); resetForm(); loadCourses(); })
    ...
    function doVerify() {
        ...
        aPatch('courses', { last_verified: new Date().toISOString() }, { id: 'eq.' + S.editId })
            .then(function () { toast('Verified.', 'success'); loadCourses(); })
            .catch(function (e) { toast('Error: ' + e.message, 'error'); });
    }
    ...
    function validateJson() {
        ...
        try {
            JSON.parse(t);
            extraJson.classList.remove('json-invalid');
            extraJson.classList.add('json-valid');
            jsonFb.textContent = 'Valid JSON';
            jsonFb.style.color = 'var(--green)';
        } catch (e) {
            extraJson.classList.remove('json-valid');
            extraJson.classList.add('json-invalid');
            jsonFb.textContent = 'Invalid JSON';
            jsonFb.style.color = 'var(--red)';
        }
    }
    ...
    function bulkImport() {
        ...
        function next() {
            if (idx >= impData.length) {
                toast('Done: ' + done + ' imported, ' + skip + ' skipped.', 'success');
                if (importBtn) { importBtn.disabled = false; importBtn.textContent = 'Import Valid Rows'; }
                loadCourses();
                return;
            }
            var r = impData[idx++];
    ...
    function doExport(fmt) {
        aGet('courses', { status: 'eq.active', limit: 10000, order: 'created_at.desc' }).then(function (r) {
            ...
            if (fmt === 'csv') {
                ...
                filename = 'courses.json'; mime = 'application/json';
            }
    ...
    function renderSugs() {
        ...
        var apprBtns = qsa('.appr-btn');
        for (var a = 0; a < apprBtns.length; a++) {
    ...
    function approveSug(id) {
        ...
        if (fields.link) fields.link.value = s.link;
        if (s.platform && fields.platform) fields.platform.value = s.platform;
